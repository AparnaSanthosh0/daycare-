...
const RecommendationTestPage = () => {
  ...
  const fetchRealChildren = useCallback(async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/recommendations/children`);
  ...
  const fetchRecommendations = async (childId) => {
    ...
    setError(null);
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/recommendations/child/${childId}?k=3&minGroupSize=2&maxGroupSize=4`);
                  ...
                  <Button 
                    onClick={async () => {
                      try {
                        ...
                        // Send recommendations to the child's parent dashboard
                        const token = localStorage.getItem('token');
                        const response = await fetch(`${API_BASE_URL}/api/recommendations/send/${selectedChild._id}`, {
                          method: 'POST',
                          headers: {
                        ...
                        if (!response.ok) {
                          ...
                          throw new Error(errorData.message || 'Failed to send recommendations');
                        }

                        const result = await response.json();
                        alert(`✅ ${result.message}\n\nRecommendations have been sent to ${result.parentsNotified.map(p => p.name).join(' and ')}'s dashboard. They can view them in their parent dashboard.`);
                        
                      } catch (err) {
                        console.error('Error sending recommendations:', err);
                        setError(err.message || 'Failed to send recommendations. Please try again.');
                    ...
                    disabled={loading}
                  >
                    {loading ? 'Sending...' : 'Send Recommendations to Parent Dashboard'}
                  </Button>
                )}
              </Box>
            </Box>
